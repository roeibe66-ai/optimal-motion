...
// boutique-clinic light theme (white, emerald, soft shadows) matching the
  ...
  return createPortal(
    <>
      <button
        ...
        // actually present.
        className={`fixed bottom-44 left-5 z-[100] w-16 h-16 rounded-full bg-gradient-to-br from-emerald-700 to-emerald-900 text-white shadow-[0_16px_32px_-10px_rgba(6,78,59,0.55)] flex items-center justify-center hover:scale-105 active:scale-95 transition-all duration-200 ease-out ${
      ...
      {isOpen && <div className="fixed inset-0 bg-stone-900/40 backdrop-blur-sm z-[100]" onClick={() => setIsOpen(false)}></div>}
      ...
      >
        ...
        <div className="px-6 pb-4 pt-1 flex items-center justify-between border-b border-stone-100 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-black text-stone-900 text-[15px]">מאמן AI אישי</h3>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
            ...
            <div key={i} className={`flex ${m.role === "user" ? "justify-start" : "justify-end"}`}>
              <div
                className={`max-w-[85%] rounded-2xl px-4 py-3 text-[13.5px] leading-relaxed whitespace-pre-wrap ${
                  m.role === "user" ? "bg-stone-100 text-stone-800" : "bg-emerald-800 text-white"
                }`}
              >
                {m.content}
              </div>
          ...
          {isLoading && (
            <div className="flex justify-end">
              <div className="bg-emerald-800/10 text-emerald-800 rounded-2xl px-4 py-3 text-xs font-bold">חושב...</div>
            </div>
          )}
        ...
        <form onSubmit={handleSubmit} className="p-4 border-t border-stone-100 flex gap-2 shrink-0 pb-[max(1rem,env(safe-area-inset-bottom))]">
          <input
            ...
            className="flex-1 bg-stone-50 border border-stone-100 rounded-2xl px-4 py-3 text-stone-900 text-sm placeholder:text-stone-400 outline-none focus:border-emerald-700 focus:ring-1 focus:ring-emerald-700/20"
          />
          <button
            type="submit"
            disabled={isLoading || !input.trim()}
            className="bg-emerald-800 text-white w-11 h-11 rounded-2xl flex items-center justify-center hover:bg-emerald-900 disabled:opacity-40 disabled:cursor-not-allowed transition-colors shrink-0"
            aria-label="שלח"
          >
            <Send size={18} />
          </button>
