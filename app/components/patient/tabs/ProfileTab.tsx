...
import { Activity, Bell, ChevronLeft, CheckCircle, Crown, Edit, Flame, Globe, LogOut, Medal, Receipt } from "lucide-react";
    ...
    <div className="animate-in fade-in duration-500">
      <div className="bg-[#1c1c1e] text-white rounded-[2rem] p-6 md:p-10 shadow-2xl relative overflow-hidden mb-8 border border-stone-800">
        ...
        <div className="relative z-10 flex items-center gap-6 mb-10">
          <div className="w-24 h-24 rounded-full bg-stone-800 flex items-center justify-center text-3xl font-black shadow-lg border border-stone-700">
            {loggedInPatient?.full_name.substring(0, 2).toUpperCase()}
          </div>
          <div>
            ...
            <p className="text-stone-400 text-sm flex items-center gap-2 mt-1">
              <Crown size={14} /> Member since {new Date(loggedInPatient?.created_at || fallbackJoinDate).toLocaleDateString("he-IL")}
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-4 mb-8">
          <div className="bg-stone-900/50 p-4 md:p-6 rounded-3xl border border-stone-800 flex flex-col items-center justify-center text-center">
            ...
            <h4 className="text-2xl md:text-3xl font-black">{streak}d</h4>
            <span className="text-[10px] md:text-xs text-stone-500 font-bold uppercase tracking-wider mt-1">Streak</span>
          </div>
          <div className="bg-stone-900/50 p-4 md:p-6 rounded-3xl border border-stone-800 flex flex-col items-center justify-center text-center">
            <Medal size={28} className={`${rank.color} mb-2`} />
            <h4 className={`text-xl md:text-2xl font-black ${rank.color}`}>{rank.name}</h4>
            <span className="text-[10px] md:text-xs text-stone-500 font-bold uppercase tracking-wider mt-1">Rank</span>
          </div>
          <div className="bg-stone-900/50 p-4 md:p-6 rounded-3xl border border-stone-800 flex flex-col items-center justify-center text-center">
            <CheckCircle size={28} className="text-teal-500 mb-2" />
            <h4 className="text-2xl md:text-3xl font-black">{totalWorkouts}</h4>
            <span className="text-[10px] md:text-xs text-stone-500 font-bold uppercase tracking-wider mt-1">Practice</span>
          </div>
        </div>
          ...
          <div className="flex justify-between items-end mb-3">
            <div>
              <span className={`text-sm font-bold flex items-center gap-2 ${rank.color}`}>
                <Medal size={16} /> {rank.name} • {totalWorkouts} pts
              </span>
            </div>
            {rank.next && <span className="text-xs text-stone-500 font-bold">{rank.max} to {rank.next}</span>}
          </div>
        ...
        <div className="space-y-2">
          ...
          <button className="w-full flex items-center justify-between p-4 hover:bg-stone-800 rounded-2xl transition-colors group">
            <div className="flex items-center gap-4">
              ...
              <div className="text-right">
                <h4 className="font-bold text-white text-sm">עריכת פרטים אישיים</h4>
                <p className="text-xs text-stone-500">שינוי סיסמה, שם וטלפון</p>
              </div>
            </div>
            <ChevronLeft size={20} className="text-stone-600 group-hover:text-stone-400" />
          </button>
          ...
          <div className="flex flex-col p-4 border-t border-stone-800 mt-2 pt-4 gap-4">
            ...
            <div className="bg-stone-900 p-4 rounded-xl flex flex-col md:flex-row gap-4 items-center border border-stone-800">
              <input
                type="time"
                value={reminderTime}
                onChange={(e) => setReminderTime(e.target.value)}
                className="text-center font-black text-white border border-stone-700 rounded-lg p-2 focus:border-teal-500 outline-none bg-stone-950"
              />
                ...
                {DAYS_OF_WEEK.map((day) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-8 h-8 rounded-lg font-bold text-xs transition-colors ${isSelected ? "bg-purple-500 text-white shadow-sm" : "bg-stone-800 text-stone-400 hover:bg-stone-700 border border-stone-700"}`}
                    >
          ...
          <div className="flex items-center justify-between p-4 border-t border-stone-800">
            ...
            <button onClick={toggleHaptics} className={`w-14 h-8 rounded-full transition-colors relative flex items-center ${hapticsEnabled ? "bg-teal-500" : "bg-stone-700"}`}>
              <div className={`w-6 h-6 bg-white rounded-full absolute shadow-sm transition-transform ${hapticsEnabled ? "left-1" : "right-1"}`}></div>
            </button>
          </div>
          ...
          <button onClick={handleLogout} className="w-full flex items-center justify-between p-4 hover:bg-red-900/20 rounded-2xl transition-colors group mt-2">
            <div className="flex items-center gap-4">
              ...
              <div className="text-right">
                <h4 className="font-bold text-red-500 text-sm">התנתק מהמערכת</h4>
              </div>
            </div>
          </button>
        </div>
